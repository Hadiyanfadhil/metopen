                ...
                <div class="p-3 bg-white card product-card">
                    ...
                    <div class="px-0 pb-0 card-body">
                        ...
                        <div class="d-flex align-items-baseline">
                            @if (!empty($item->discount_price))
                                <h6 class="mb-0 text-danger">Rp {{ number_format($item->selling_price - $item->discount_price, 0, ',', '.') }} </h6>
                            @else
                                <h6 class="mb-0 text-danger">Rp {{ number_format($item->selling_price, 0, ',', '.') }} </h6>
                            @endif
                            @if (!empty($item->discount_price))
                                <p class="mb-0 h6 ms-2 text-black-50 text-decoration-line-through"> Rp {{ number_format($item->selling_price, 0, ',', '.') }} </p>
                            @endif
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function showProductPrice(discountPrice,sellingPrice){
        ...
        if(discountPrice && discountPrice != 0){
            productPriceHtml = `
                <h6 class="mb-0 text-danger">Rp ${sellingPrice - discountPrice} </h6>
                <p class="mb-0 h6 ms-2 text-black-50 text-decoration-line-through"> Rp ${sellingPrice} </p>
            `;
        }else{
            productPriceHtml = `
            <h6 class="mb-0 text-danger">Rp ${sellingPrice} </h6>
            `;
        }
